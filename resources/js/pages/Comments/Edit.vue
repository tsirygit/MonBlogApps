<template>
    <Head title="modifier votre commentaire..." />
    <NavbarLayout />

    <div class="flex items-center justify-center">
        <div class="w-full max-w-sm rounded-lg bg-slate-900 p-4 shadow-lg lg:max-w-lg">
            <h1 class="mb-2 text-center text-xl font-bold text-white md:text-2xl">modifier le commentaire</h1>
            <div class="mt-8 justify-items-center">
                <form @submit.prevent="submit" class="w-full">
                    ...
                    <div class="mx-6 mt-4 text-center text-xl">
                        <Link :href="route('homepage')" class="text-white hover:underline">Annuler la modification du commentaire</Link>
                    </div>
                </form>
            </div>
        </div>
...
<script setup>
/**
 * Composant : EditComment.vue
 *
 * Permet à l'utilisateur de modifier un commentaire existant.
 * Utilise Inertia.js pour envoyer une requête PATCH à la route 'comment.update'.
 */
 ...
 *
 * @prop {Object} comment - Données du commentaire à modifier
 */
...
/**
 * Initialisation du formulaire avec le contenu du commentaire
 */
const form = useForm({
 ...
 * Envoie une requête PATCH à la route 'comment.update'
